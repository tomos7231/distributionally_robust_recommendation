...
from src.optimization import make_covariance_matrix, optimize_ro
...
def main(cfg: MyConfig):
    ...
    # 最適化
    result, mean_time, std_time = optimize_ro(
        alpha=cfg.optimization.alpha,
        gamma_mu=cfg.optimization.gamma_mu,
        gamma_sigma=cfg.optimization.gamma_sigma,
        c_mu=cfg.optimization.c_mu,
        c_sigma=cfg.optimization.c_sigma,
    ...
    # 評価
    evaluate(result, logger, train_df, test_df, cfg.optimization.N, cfg.evaluation.thres_rating)
    ...
    with open(path_exp / "result.txt", "a") as f:
        # スペース区切りで保存
        f.write(
            f"{cfg.seed} {cfg.optimization.gamma_mu} {cfg.optimization.gamma_sigma} {cfg.optimization.alpha} {f1} {diversity} {var_hit_item} {var_recommended} {gini} {mean_time} {std_time}\n"
        )
